...
const EventTrackerGraph: React.FC<{
  ...
  ({ rtRanking, ranking, eventId, mobileTable = false, charaId }) => {
    const theme = useTheme();
    const { t } = useTranslation();
    const { region } = useRootStore();

    const { getGraph, getChapterGraph } = useEventTrackerAPI(region);
    ...
    const handleFetchGraph = useCallback(async () => {
      const data: EventRankingResponse[] = charaId
        ? await getChapterGraph(eventId, charaId, ranking)
        : await getGraph(eventId, ranking);
      ...
      setIsShowChart(true);
    }, [charaId, eventId, getChapterGraph, getGraph, ranking]);
